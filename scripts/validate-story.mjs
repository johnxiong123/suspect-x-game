...
const chapters = {};
for (const f of files) chapters[f.replace('.json', '')] = read(`data/chapters/${f}`);
...
for (const [chId, ch] of Object.entries(chapters)) {
  ...
  for (const [nid, node] of Object.entries(ch.nodes)) {
    ...
    const collectClues = (eff) => (eff?.clues || []);
    for (const c of collectClues(node.onEnter)) if (!clueIds.has(c)) errors.push(`${where}: onEnter 线索 "${c}" 未定义`);

    const targets = [];
    if (node.next) targets.push(node.next);
    for (const ch2 of node.choices || []) {
      if (ch2.goto) targets.push(ch2.goto);
      for (const c of collectClues(ch2.effects)) if (!clueIds.has(c)) errors.push(`${where}: choice 线索 "${c}" 未定义`);
    }
    ...
    const exits = (node.next ? 1 : 0) + ((node.choices || []).length ? 1 : 0) + (node.ending ? 1 : 0) + (node.chapterEnd ? 1 : 0);
    if (exits === 0) errors.push(`${where}: 节点没有任何出口（next/choices/ending/chapterEnd）`);
  }
}
